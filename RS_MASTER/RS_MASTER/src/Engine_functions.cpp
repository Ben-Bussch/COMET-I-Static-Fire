...
#define PYROPINCOUNTDOWN 10000
#define NOXENGSERVOCOUNTDOWN 10000
#define IPAENGSERVOCOUNTDOWN 10100
...
unsigned long fireSequence(unsigned long FireStartTime, unsigned long clk_time, int* FireSeq, int PyroPin)
{
  ...
  if(countdown >= PYROPINCOUNTDOWN && countdown < PYROPINCOUNTDOWN+100 && *FireSeq == 1){
    digitalWrite(PyroPin, HIGH);
    
    *FireSeq = 2;
  }

  if(countdown  >= NOXENGSERVOCOUNTDOWN && *FireSeq == 2){
    NoxEngServo.writeMicroseconds(NoxEngStartPPM+NoxdeltaPPM);
    *FireSeq = 3;
  }
  if(countdown >= IPAENGSERVOCOUNTDOWN && *FireSeq == 3){
